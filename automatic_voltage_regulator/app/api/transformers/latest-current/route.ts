...
export async function GET(req: NextRequest) {
  try {
    ...
    // Use sensorId D151 for current, fetch the latest (n=1), endTime = now
    const now = new Date();
    ...
    if (Array.isArray(result) && result.length > 0) {
      return NextResponse.json({ current: result[0].value, time: result[0].time });
    } else {
      return NextResponse.json({ error: 'No current data found' }, { status: 404 });
    }
  } catch (error: any) {
